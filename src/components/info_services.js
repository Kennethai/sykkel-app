...
class InfoService {
  ...
  utstyrr(kunde, success) {
    connection.query(
      'SELECT utleid_sykkel.utleie_id, COUNT(utleid_sykkel.sykkel_id) as sykler, utleid_utstyr.utleie_id, COUNT(utleid_utstyr.utleie_id) as utstyr FROM kunde,(utleie LEFT JOIN utleid_utstyr ON utleid_utstyr.utleie_id = utleie.utleie_id) LEFT JOIN utleid_sykkel ON utleid_sykkel.utleie_id = utleie.utleie_id WHERE kunde.kunde_nr = utleie.kunde_nr AND k_fornavn=? AND k_etternavn=? AND k_tlf=? GROUP BY utleid_sykkel.utleie_id, utleid_utstyr.utleie_id UNION  SELECT utleid_sykkel.utleie_id, COUNT(utleid_sykkel.sykkel_id) as sykler, utleid_utstyr.utleie_id, COUNT(utleid_utstyr.utleie_id) as utstyr FROM kunde, (utleie RIGHT JOIN utleid_utstyr ON utleid_utstyr.utleie_id = utleie.utleie_id) RIGHT JOIN utleid_sykkel ON utleid_sykkel.utleie_id = utleie.utleie_id WHERE kunde.kunde_nr = utleie.kunde_nr AND k_fornavn=? AND k_etternavn=? AND k_tlf=? GROUP BY utleid_sykkel.utleie_id, utleid_utstyr.utleie_id',
      [kunde.fornavn, kunde.etternavn, kunde.tlf],
      (error, results) => {
        if (error) return console.error(error);
        success(results);
